class PngExporter {
    ...
    constructor() {
        ...
        // Use location.origin + '/' to ensure correct base path for worker imports, even with hash routing
        const origin = window.location.origin + (window.location.origin.endsWith('/') ? '' : '/');
        const workerBlob = new Blob([`(${PngExporter.WORKER_STR})('${origin}')\n\n`], {
